...
For one-click simulator demos, use the shared `FitnessRPGDemo` Xcode scheme or run `bash native/scripts/demo-seed-simulator-smoke.sh`; pass `--screenshot /private/tmp/fitnessrpg-demo-smoke.png` for one UI capture or `--screenshots-dir /private/tmp/fitnessrpg-demo-gallery` for the full History/detail/Today/Memory/archive gallery plus `manifest.md` and `index.html`. After one full run, add `--skip-build` for faster screenshot recapture. See `docs/validation/demo-seed-runbook.md`.
