...
- LiteRT-LM / Gemma SDK execution can now start with `bash native/scripts/litertlm-integration-checklist.sh`, then `bash native/scripts/litertlm-real-device-preflight.sh`, and follow `docs/validation/litertlm-real-device-runbook.md` before linking the Swift package, placing `ModelResources/gemma-4-E2B-it.litertlm`, and setting `FITNESSRPG_ENABLE_LITERTLM`.
