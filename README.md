...
The shared core now includes a local model runtime scaffold, adapter boundary, SDK-independent resource preflight layer, and a Gemma E2B resource catalog. It builds bounded context from Today readiness, the current quest, and recent Memory Review entries, checks provider resource requirements from platform observations, calls an interchangeable draft provider, then validates draft coach text before it can be accepted. The iOS DEBUG diagnostics path uses the Core catalog to scan `Bundle.main` for `gemma-e2b.task` and `tokenizer.model`; invalid, unavailable, or missing model output falls back to deterministic safety copy.
